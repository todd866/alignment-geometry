...
\begin{abstract}
We study the probability of simultaneous alignment events on product statistical manifolds, with applications to synchronization in coupled oscillator systems. For $M$ random variables drawn independently from a common exponential family on the circle (von Mises distributions with concentration $\kappa$), the probability that all variables fall within tolerance $\varepsilon$ of each other scales as $p_1(\varepsilon, \kappa)^{M-1}$, where $p_1$ is the single-variable window probability. The exponent $(M-1)$ rather than $M$ arises from quotient geometry: alignment is invariant under global rotation, so the constraint has \emph{codimension} $M-1$ in the $M$-torus after quotienting by the diagonal $S^1$ action. We prove this product formula, characterize the single-variable probability $p_1$ in terms of Fisher information on the circle, and derive the resulting scaling law for expected hitting times: $\tau \propto p_1^{-(M-1)}$. The framework provides a coordinate-invariant foundation for understanding how ``coordination depth'' (the number of components requiring simultaneous alignment) determines waiting times in stochastic systems. Applications include neural synchronization, molecular coordination, and distributed consensus.
\end{abstract}
...
\begin{enumerate}
    \item The alignment probability factorizes as $P(\text{align}) = p_1(\varepsilon, \kappa)^{M-1}$, where $p_1$ is the single-variable window probability and $\kappa$ is the concentration parameter.
    ...
    \item The single-variable probability $p_1(\varepsilon, \kappa)$ is determined by the Fisher information geometry of the von Mises family, connecting alignment rates to statistical distinguishability.

    \item Expected hitting times scale as $\tau \propto p_1^{-(M-1)}$, exponential in coordination depth $M$.
\end{enumerate}
...
The Fisher information for the von Mises family with respect to $\kappa$ is:
\begin{equation}
I(\kappa) = \mathbb{E}\left[\left(\frac{\partial \log p}{\partial \kappa}\right)^2\right] = \frac{I_2(\kappa)}{I_0(\kappa)} - \left(\frac{I_1(\kappa)}{I_0(\kappa)}\right)^2 = A(\kappa) - r^2
\end{equation}
where $A(\kappa) = I_2(\kappa)/I_0(\kappa)$. Using the Bessel recurrence $I_2(\kappa) = I_0(\kappa) - (2/\kappa)I_1(\kappa)$:
\begin{equation}
I(\kappa) = 1 - \frac{2r}{\kappa} - r^2 = 1 - r\left(\frac{2}{\kappa} + r\right)
\end{equation}

For large $\kappa$ (high concentration), $r \to 1$ and $I(\kappa) \to 0$: highly concentrated distributions are easy to distinguish from slight perturbations in $\kappa$. For small $\kappa$ (near uniform), $r \to 0$ and $I(\kappa) \to 1$.

\begin{remark}[Fisher information and circular variance]
The circular variance is $V = 1 - r$. High Fisher information (near 1) corresponds to high circular variance (diffuse distribution); low Fisher information corresponds to low circular variance (concentrated distribution). This inverse relationship connects statistical distinguishability to phase dispersion.
\end{remark}
...
\textbf{High concentration ($\kappa \gg 1$).} For large $\kappa$, the von Mises distribution approaches a wrapped Gaussian with variance $1/\kappa$. The Gaussian approximation gives:
\begin{equation}
p_1(\varepsilon, \kappa) \approx \text{erf}\left(\frac{\varepsilon\sqrt{\kappa}}{2}\right) = \frac{2}{\sqrt{\pi}}\int_0^{\varepsilon\sqrt{\kappa}/2} e^{-t^2}\,dt
\end{equation}
...
\begin{equation}
A_\varepsilon = \{(\theta_1, \ldots, \theta_M) : |\theta_i - \theta_j| < \varepsilon \text{ for all } i, j\}
\end{equation}
This is the set where all phases lie within $\varepsilon$ of each other---a ``diagonal band'' in the torus. Throughout, $\varepsilon$ denotes the \textbf{full window width}, so that individual phases must lie within $\pm\varepsilon/2$ of the reference.
...
\begin{itemize}
    \item The alignment region becomes $\{|\psi_i| < \varepsilon : i = 1, \ldots, M-1\}$
...
Alternatively: the alignment constraint imposes $M-1$ independent conditions (all pairwise differences $|\theta_i - \theta_j| < \varepsilon$ are equivalent to $|\psi_i| < \varepsilon$ for $i = 1, \ldots, M-1$). This gives codimension $M-1$ in $T^M$, or equivalently, dimension 1 (the free global phase).

\begin{remark}[Why $(M-1)$, not $\binom{M}{2}$]
There are $\binom{M}{2}$ pairwise constraints $|\theta_i - \theta_j| < \varepsilon$, but these are not independent. Given $\theta_1$ as reference, the constraints $|\theta_i - \theta_1| < \varepsilon$ for $i = 2, \ldots, M$ imply $|\theta_i - \theta_j| < 2\varepsilon$ by the triangle inequality. The $M-1$ reference-based constraints determine the alignment (up to window width).
\end{remark}
...
\begin{equation}
P(A_\varepsilon) = p_1(\varepsilon, \kappa)^{M-1}
\end{equation}
where $p_1(\varepsilon, \kappa)$ is the single-variable window probability.
\end{theorem}

\begin{proof}
By rotational invariance, we may condition on $\theta_1 = 0$ (or equivalently, use $\theta_1$ as the reference phase). The alignment event $A_\varepsilon$ then requires each of $\theta_2, \ldots, \theta_M$ to lie in the window $(-\varepsilon/2, \varepsilon/2)$.

Since the $\theta_i$ are independent and identically distributed:
\begin{align}
P(A_\varepsilon) &= P(\theta_2 \in (-\varepsilon/2, \varepsilon/2), \ldots, \theta_M \in (-\varepsilon/2, \varepsilon/2) \,|\, \theta_1 = 0) \\
&= \prod_{i=2}^M P(\theta_i \in (-\varepsilon/2, \varepsilon/2)) \\
&= p_1(\varepsilon, \kappa)^{M-1}
\end{align}
The exponent is $(M-1)$ because $\theta_1$ serves as the reference; only the remaining $M-1$ phases must independently fall within the tolerance window.
\end{proof}
...
\begin{equation}
\mathbb{E}[\tau] \propto \frac{1}{\Delta\omega} \cdot p_1(\varepsilon, \kappa)^{-(M-1)}
\end{equation}
Equivalently, $\log \mathbb{E}[\tau] \propto (M-1) \log(1/p_1)$: logarithmic hitting time is linear in coordination depth.
\end{corollary}

\begin{proof}
For a renewal process with success probability $p$ per attempt and attempt rate $\Delta\omega$, the expected waiting time is $\mathbb{E}[\tau] = 1/(\Delta\omega \cdot p)$. Substituting $p = P(A_\varepsilon) = p_1^{M-1}$ yields the result.
\end{proof}

\begin{remark}[Geometric interpretation]
The $(M-1)$ exponent has a clean geometric meaning: the alignment constraint removes $M-1$ degrees of freedom from the $M$-torus. Equivalently, after quotienting by global rotation, the relative-phase space is $(M-1)$-dimensional, and alignment requires all $M-1$ relative phases to independently satisfy a window constraint.
\end{remark}
...
\begin{table}[ht]
\centering
\caption{Qualitative $\alpha$ regimes by topology (schematic)}
\label{tab:alpha}
\begin{tabular}{lcc}
\toprule
Topology & Inter-module structure & Predicted $\alpha$ \\
\midrule
Modular (weak inter) & Sparse, weak & $\alpha \approx 1$ (near-independent) \\
Modular (moderate inter) & Sparse, moderate & $\alpha \approx 0.3$--$0.5$ \\
All-to-all & Dense, strong & $\alpha \approx 0.5$--$0.8$ \\
Sparse random & Very sparse & $\alpha \approx 0.2$--$0.4$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent These are order-of-magnitude expectations based on the perturbative analysis, not formal bounds; exact values depend on specific coupling parameters.
...
\begin{equation}
p_1(\pi/3, 10) \approx \text{erf}\left(\frac{(\pi/3)\sqrt{10}}{2}\right) = \text{erf}(1.66) \approx 0.98
\end{equation}

\textbf{Alignment probability without coupling} ($\alpha = 1$):
\begin{equation}
P_{\alpha=1} = (0.98)^7 \approx 0.87
\end{equation}

\textbf{Alignment probability with coupling} ($\alpha = 0.4$):
\begin{equation}
P_{\alpha=0.4} = (0.98)^{0.4 \times 7} = (0.98)^{2.8} \approx 0.94
\end{equation}

The coupling correction increases alignment probability from 87\% to 94\%---a modest effect when $p_1$ is already high, but the relative change in $\log P$ is significant:
\begin{equation}
\frac{\log P_{\alpha=0.4}}{\log P_{\alpha=1}} = \frac{0.4 \times 7 \times \log(0.98)}{7 \times \log(0.98)} = 0.4
\end{equation}

For lower $p_1$ (harder alignment), the coupling correction has larger absolute effect.
...
This paper provides:
\begin{enumerate}
    \item A rigorous derivation of the $(M-1)$ exponent for alignment probabilities on product manifolds (Theorem~\ref{thm:alignment})

    \item A geometric explanation: the exponent equals the codimension of the alignment constraint after quotienting by global rotation

    \item Connection to Fisher information: alignment rates are determined by the statistical geometry of the component distributions

    \item Hitting time scaling (Corollary~\ref{cor:hitting}): $\tau \propto p_1^{-(M-1)}$, exponential in coordination depth
\end{enumerate}
...
The probability of simultaneous alignment across $M$ independent circular random variables scales as $p_1^{M-1}$, where $p_1$ is the single-component window probability. The exponent $(M-1)$---not $M$---arises from the quotient geometry: one component serves as reference, leaving $M-1$ independent constraints. This provides a coordinate-invariant, information-geometric foundation for understanding how coordination depth determines waiting times in stochastic systems.

The key insight is that ``coordination depth'' is a geometric quantity: it counts the codimension of the alignment constraint on the product manifold. Fisher information enters through the single-component window probability, connecting statistical distinguishability to alignment rates.
